...
public class PythagorasTree {
	...
	private void drawPythagorasTree(double length, float x, float y, int level, double angle, PApplet marker) {
		...
		if (level == 1) {
			...
			marker.quad(x, y, (float) (x + length * Math.cos(angle)), (float) (y - length * Math.sin(angle)), x3, y3,
					x4, y4);
		} else {
			length /= Math.sqrt(2);
			drawPythagorasTree(length, x, y, level - 1, angle + Math.PI / 4, marker);
			drawPythagorasTree(length, (float) (x + length * Math.cos(angle)), (float) (y - length * Math.sin(angle)),
					level - 1, angle - Math.PI / 4 + Math.PI, marker);
		}
	}
